		...
		if (sscanf(argv[1], "%d.%d.%d.%d", &a, &b, &c, &d))
		{
			...
			if (strstr(argv[2], ".txt") != nullptr)
			{
				...
				else
				{
					...
					filename.assign(argv[2], argv[2] + sizeof(argv[2]));
					extension.assign(txt, txt + 5);
					filename.insert(filename.begin() + sizeof(argv[2]), extension.begin(), extension.end());
					//putting .txt in here so it can be interpreted later on
			...
			else if (strstr(argv[2], ".bin") != nullptr)
			{
				...
				else
				{
					...
					filename.assign(argv[2], argv[2] + sizeof(argv[2]));
					//putting .bin in here so it can be interpreted late on
					fdata.insert(fdata.begin(), 1);
					fdata.insert(fdata.begin() + 1, filename.begin(), filename.end());
					fdata.insert(fdata.begin() + 1 + sizeof(filename), ' ');
